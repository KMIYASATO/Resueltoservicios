      ...
      {active ? (
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <Card className="h-fit p-5">
            <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-brand-600">Panel mock profesional</p><h1 className="mt-1 font-display text-3xl font-bold text-neutral-950">{active.assignmentMode === "open" ? "Solicitud publicada" : `Solicitud para ${active.professional.name}`}</h1><p className="mt-2 text-neutral-600">{active.need}</p></div><RequestStatusBadge status={active.status} /></div>
