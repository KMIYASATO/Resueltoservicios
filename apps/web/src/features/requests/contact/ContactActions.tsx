...
export function ContactActions({ request }: { request: ServiceRequest }) {
  ...
  const contact = selectedOffer ? getProfessionalContact(selectedOffer.professional.id, request.status, request.acceptedOfferId) : null;
  if (!contact) {
  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-success-100 bg-success-100/60 p-4">
      <div>
        <h2 className="font-display text-xl font-bold text-neutral-950">Coordina con {selectedOffer?.professional.name.split(" ")[0] ?? request.professional.name.split(" ")[0]}</h2>
      ...
      {mockContact === "whatsapp" ? <div className="rounded-xl bg-white p-3 text-sm leading-6 text-neutral-700"><p className="font-semibold text-neutral-950">WhatsApp simulado abierto</p><p>{getWhatsAppMessage(selectedOffer?.professional.name ?? request.professional.name)}</p><p className="text-xs text-neutral-500">No se abre `wa.me` con números falsos para evitar errores.</p></div> : null}
    </div>
  );
}
